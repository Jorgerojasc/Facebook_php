	...
	class Conexion{
	    ...
	    public function select($attr, $table, $where = '', $orderBy = '', $limit = false ){
	    	...
	    	if($this->getStatusConexion()){
				...
				if($result->num_rows > 0) {
					...
					$response = 'No se encontro ningun registro';
				}
			} else {
		...
		public function delete( $table, $where){
			...
			$result = $this->link->query($stmt) or die($this->link->error);
			if($result == 'No se encontró ningún registro'){
				return $result = 'No se encontro ningun registro!';
			}
			else{
				return $result=True;
			}
		...
		public function query($stmt){
			...
			if(is_bool($result)){
				...
				if($result->num_rows > 0) {
					...
					return $response = 'No se encontro ningun registro';
